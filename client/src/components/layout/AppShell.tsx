...
import { Image, Upload, FolderOpen, Users, Settings, ChevronRight, ChevronLeft } from 'lucide-react';
...
export function AppShell() {
  ...
  const isOwner = user?.idTokenClaims?.roles?.includes('owner') ?? true; // default true for DevAuth
  ...
  return (
    <GlobalDropZone>
      <div className="min-h-screen flex" style={{ background: 'var(--content-bg)', color: 'var(--text-primary)' }}>
        <aside className={`flex-shrink-0 flex flex-col transition-all duration-200 ${collapsed ? 'w-16' : 'w-60'}`} style={{ background: 'var(--sidebar-bg)', borderRight: '1px solid var(--border)' }}>
          ...
          {!collapsed && usage && (
            <div className="px-4 py-2" style={{ borderTop: '1px solid var(--border)' }}>
              <div className="flex justify-between text-xs mb-1" style={{ color: 'var(--text-muted)' }}>
                <span>{usage.usedFormatted}</span>
                <span>{usage.quotaFormatted}</span>
              </div>
