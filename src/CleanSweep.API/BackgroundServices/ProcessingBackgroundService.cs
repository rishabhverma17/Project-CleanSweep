...
public class ProcessingBackgroundService : BackgroundService
{
    ...
    {
        _logger.LogInformation("ProcessingBackgroundService started, listening on queue '{Queue}'", _queueOptions.MediaProcessingQueue);
        ...
        while (!stoppingToken.IsCancellationRequested)
        {
            try
            {
                ...
                // Download blob to temp file on disk (not RAM — handles any file size)
                ...
                try
                {
                    // Stream from blob → disk (~4 MB buffer, not full file in RAM)
